...
namespace AdvancedBuildingControl.Extensions
{
    public abstract partial class ExtendedInfoSectionBase : InfoSectionBase
    {
        ...
        public ValueBindingHelper<T> CreateBinding<T>(string key, T initialValue)
        {
            var helper = new ValueBindingHelper<T>(
                new(Mod.Id, key, initialValue, new GenericUIWriter<T?>())
            );
        ...
        )
        {
            ...
            var trigger = new TriggerBinding<T>(
                Mod.Id,
                setterKey,
                helper.UpdateCallback,
                GenericUIReader<T>.Create()
            );
        ...
        {
            var binding = new GetterValueBinding<T>(
                Mod.Id,
                key,
                getterFunc,
        ...
        {
            var binding = new TriggerBinding<T1, T2>(
                Mod.Id,
                key,
                action,
                GenericUIReader<T1>.Create(),
                GenericUIReader<T2>.Create()
            );
        ...
        {
            var binding = new TriggerBinding<T1, T2, T3>(
                Mod.Id,
                key,
                action,
                GenericUIReader<T1>.Create(),
                GenericUIReader<T2>.Create(),
                GenericUIReader<T3>.Create()
            );
        ...
        {
            var binding = new TriggerBinding<T1, T2, T3, T4>(
                Mod.Id,
                key,
                action,
                GenericUIReader<T1>.Create(),
    ...
    public class GenericUIWriter<T> : IWriter<T>
    {
        ...
        private static void WriteObject(IJsonWriter writer, Type type, object obj)
        {
            ...
            foreach (
                var propertyInfo in properties.Where(x => !x.HasAttribute<WriterIgnoreAttribute>())
            )
            {
    ...
    {
        private static readonly Dictionary<Type, object> _readers =
            (Dictionary<Type, object>)
                typeof(ValueReaders)
                    .GetField("s_Readers", BindingFlags.NonPublic | BindingFlags.Static)
                    .GetValue(null);
        ...
        private static object ReadObject(IJsonReader reader, Type type)
        {
            ...
            {
                if (
                    !propertyInfo.HasAttribute<ReaderIgnoreAttribute>()
                    && reader.ReadProperty(propertyInfo.Name)
                )
                {
            ...
            {
                if (
                    !fieldInfo.HasAttribute<ReaderIgnoreAttribute>()
                    && reader.ReadProperty(fieldInfo.Name)
                )
                {
